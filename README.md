...
Потому что те задачи которые описаны в тестовом задании более менее надежно можно решить с помощью Postgres. Остальное - либо очень бедные простые базы либо очень дорогие (для коммерческих целей) решения такие как Oracle или Microsoft SQL Server. 
...
Все идемпотентные операции реализованы.

Создание ивентов полностью асинхронно.

Nodejs I/O нигде не блокируются. Основная нагрузка падает на базу данных. А Postgres очень надежный и быстрый.
